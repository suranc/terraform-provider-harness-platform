		...
		Steps: []resource.TestStep{
			...
			},
			{
				PreConfig: func() {
					...
					c := acctest.TestAccProvider.Meta().(*nextgen.APIClient)
					_, _, err := c.ConnectorsApi.DeleteConnector(context.Background(), c.AccountId, id, nil)
...
func testAccGetConnector(resourceName string, state *terraform.State) (*nextgen.ConnectorInfo, error) {
	...
	resp, _, err := c.ConnectorsApi.GetConnector(context.Background(), c.AccountId, id, options)
	if err != nil {
		return nil, err
	}
